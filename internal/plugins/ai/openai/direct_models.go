...
func FetchModelsDirectly(ctx context.Context, baseURL, apiKey, providerName string) ([]string, error) {
	...
	if baseURL == "" {
		return nil, fmt.Errorf("%s", fmt.Sprintf(i18n.T("openai_api_base_url_not_configured"), providerName))
	}

	// Build the /models endpoint URL
	fullURL, err := url.JoinPath(baseURL, "models")
	if err != nil {
		return nil, fmt.Errorf("%s", fmt.Sprintf(i18n.T("openai_failed_to_create_models_url"), err))
	}
	...
	if resp.StatusCode != http.StatusOK {
		...
		if readErr != nil {
			if len(bodyString) > 0 {
				return nil, fmt.Errorf("%s", fmt.Sprintf(i18n.T("openai_unexpected_status_code_read_error_partial"),
					resp.StatusCode, providerName, readErr, bodyString))
			}
			return nil, fmt.Errorf("%s", fmt.Sprintf(i18n.T("openai_unexpected_status_code_read_error"),
				resp.StatusCode, providerName, readErr))
		}
		return nil, fmt.Errorf("%s", fmt.Sprintf(i18n.T("openai_unexpected_status_code_with_body"),
	...
	return nil, fmt.Errorf("%s", fmt.Sprintf(i18n.T("openai_unable_to_parse_models_response"), truncatedBody))
}
