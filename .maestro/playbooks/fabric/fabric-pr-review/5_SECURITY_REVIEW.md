...
- [x] **Server mode security**:
  ...
  - Note: Revalidated `internal/server/chat.go`; SSE frames are still emitted as JSON payloads under a single `data:` envelope, and hardened the response headers/error path so streamed responses now use `no-store` caching semantics, redact backend/internal error details to a generic client-safe message, and stop on error instead of appending a misleading completion frame.
  - Note: Hardened API-key-protected REST server mode with an in-process per-client limiter (60 requests/minute, `429` + `Retry-After`) so public binds no longer rely entirely on an external reverse proxy for brute-force and burst-abuse resistance.
  - Note: Updated `docs/rest-api.md` and `README.md` to document the new built-in throttling behavior for API-key-protected server mode while still recommending a reverse proxy for stricter edge controls.
  - Note: Added regression coverage in `internal/server/storage_handler_test.go`, `internal/server/patterns_test.go`, `internal/server/chat_test.go`, `internal/server/auth_test.go`, and `internal/server/rate_limit_test.go`, then reran `go test ./internal/server`.
