...
<div class="px-4 pb-4 flex gap-4 items-center">
	...
	<div class="flex-1">
		<!-- Add flex-1 to push search to right -->
	...
	<div class="w-48">
		...
		<Input type="text" bind:value={searchText} placeholder="Search patterns..." />
	</div>
</div>
```
...
$: sortedPatterns =
	sortBy === 'alphabetical'
		...
		: [
				...filteredPatterns
					.filter((p) => $favorites.includes(p.patternName))
					.sort((a, b) => a.patternName.localeCompare(b.patternName)),
				...filteredPatterns
					.filter((p) => !$favorites.includes(p.patternName))
					.sort((a, b) => a.patternName.localeCompare(b.patternName))
			];
