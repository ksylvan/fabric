<script lang="ts">
	...
	let tagFilterRef: TagFilterPanel = $state(undefined!);
...
<div class="flex h-[85vh]">
	...
	<div
		class={cn(
			...
			isTagPanelOpen ? 'w-[75vw]' : 'w-full max-w-[95vw] mx-auto'
		)}
	>
		<!-- Header with grid layout -->
		<div
			class="grid grid-cols-[auto_auto_1fr_auto] items-center p-4 border-b border-primary-700/30 sticky top-0 bg-primary-800 z-10"
		>
			<!-- Left column: Title -->
		...
		{#if selectedTags.length > 0}
			<div class="px-4 pb-2 pt-2 border-b border-primary-700/30">
				<div
					class="text-sm text-white/70 bg-primary-700/30 rounded-md p-2 flex justify-between items-center"
				>
					<div class="flex flex-wrap gap-1 items-center">
						<span class="mr-1">Tags:</span>
						{#each selectedTags as tag}
							<div
								class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-primary-600/40 text-primary-200 border border-primary-500/30"
							>
								{tag}
								<button
					...
					{#each filteredPatterns as pattern}
						...
						>
							<div class="flex justify-between items-start mb-2">
								<h3
									class="pattern-name font-bold text-base text-primary-200 leading-tight break-all overflow-hidden pr-2 w-[85%]"
								>
									{pattern.Name}
								</h3>
								<span
									...
									onclick={(e: MouseEvent) => {
										e.stopPropagation();
										toggleFavorite(pattern.Name);
									}}
									onkeydown={(e: KeyboardEvent) => {
										e.stopPropagation();
										if (e.key === 'Enter' || e.key === ' ') toggleFavorite(pattern.Name);
									}}
									class="focus:outline-none ml-1 mt-0.5 cursor-pointer"
									aria-label={$favorites.includes(pattern.Name)
							...
							{#if pattern.tags && pattern.tags.length > 0}
								<div class="flex flex-wrap gap-1 mt-2">
									{#each pattern.tags as tag}
										<span
											class="inline-flex items-center px-1 py-0.25 rounded-full text-[8px] font-medium bg-primary-600/40 text-primary-200 border border-primary-500/30"
										>
											{tag}
										</span>
									{/each}
