			...
			<ul class="flex items-center space-x-8">
				{#each navItems as { href, label }}
					<li>
						<a
							{href}
							class="text-sm font-medium transition-colors hover:text-primary {currentPath === href
								? 'text-primary'
								: 'text-foreground/60'}"
						>
							{label}
		...
		<div class="container md:hidden" transition:fade={{ duration: 200 }}>
			<nav class="flex flex-col space-y-4 p-4">
				{#each navItems as { href, label }}
					<a
						{href}
						class="text-base font-medium transition-colors hover:text-primary {currentPath === href
							? 'text-primary'
							: 'text-foreground/60'}"
...
<Modal show={showHelpModal} on:close={() => (showHelpModal = false)}>
	<HelpModal on:close={() => (showHelpModal = false)} />
</Modal>

<Modal show={showPatternTilesModal} on:close={() => (showPatternTilesModal = false)}>
	<PatternTilesModal
