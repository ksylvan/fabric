...
<div class="chat-container flex gap-0 p-2 w-full h-screen">
	...
	<aside class="flex flex-col gap-2 pr-2 left-column" style="width: {leftColumnWidth}%">
		...
		<!-- Message Input -->
		<div
			class="bg-background/5 rounded-lg overflow-hidden"
			style="height: {messageInputHeight}%; max-height: {MAX_INPUT_HEIGHT}%"
		>
			<ChatInput />
		</div>
	...
	<div class="flex flex-col gap-2" style="width: {rightColumnWidth}%">
		...
		<div class="flex-1 flex flex-col min-h-0">
			<!-- Chat History -->
			<div
				class="flex-1 min-h-0 bg-background/5 rounded-lg overflow-y-scroll scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent hover:scrollbar-thumb-white/20"
			>
				<ChatMessages />
				<div class="h-32"></div>
				<!-- Spacer div to ensure scrolling works properly -->
			</div>
		</div>
	</div>
</div>
