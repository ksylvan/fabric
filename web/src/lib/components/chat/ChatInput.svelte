<script lang="ts">
	...
	let fileInputEl: HTMLInputElement = $state(undefined!);
	...
	async function handleFileUpload(_e: Event) {
		...
		try {
			...
			messageStore.update((messages) => messages.filter((m) => m.format !== 'loading'));
		} catch (error) {
	...
	async function readFileContent(file: File): Promise<string> {
		...
		if (file.type === 'application/pdf') {
			try {
				...
				const errorMessage =
					error instanceof Error ? error.message : 'Unknown error during PDF conversion';
	...
	async function handleSubmit() {
		...
		try {
			...
			const contentWithFiles =
				contentsForProcessing.length > 0
