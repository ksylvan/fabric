<script lang="ts">
	...
	const settings = [
		...
		},
		{
			...
			max: 1,
			step: 0.01,
			tooltip: 'Reduces repetition of the same words'
		},
		{
			key: 'presence',
			...
			max: 1,
			step: 0.01,
			tooltip: 'Reduces repetition of similar topics'
		}
	] as const;
</script>
...
<div class="w-full" use:clickOutside={handleClickOutside}>
	...
	>
		...
		<span
			class="transform transition-transform duration-200 opacity-70 ml-1 text-xs"
				...
				<div class="group">
					<div class="flex justify-between items-center mb-0.5">
						<Tooltip text={setting.tooltip} position="right">
							<Label
								class="text-[10px] text-white/70 cursor-help group-hover:text-white/90 transition-colors"
								>{setting.label}</Label
							>
						</Tooltip>
						<span
							class="text-[10px] font-mono text-white/50 group-hover:text-white/70 transition-colors"
						>
							{typeof $modelConfig[setting.key] === 'number'
								? $modelConfig[setting.key].toFixed(2)
								: $modelConfig[setting.key]}
						</span>
					</div>
					<Slider
